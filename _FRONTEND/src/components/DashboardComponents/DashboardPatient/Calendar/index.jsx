...
export default function Index() {
  ...
  const [startOfWeekDate, setStartOfWeekDate] = useState(startOfWeek(currentDate, { weekStartsOn: 1 }));
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-[#E3E2E2] rounded-full">
      <button 
        ...
        <div className="flex flex-1 overflow-x-auto">
          <div className="flex space-x-8">
            {days.map((day) => {
              ...
              const dayClass = `flex flex-col items-center justify-center w-12 h-12 text-center rounded-[30%] cursor-pointer ${
              ...
              return (
                ...
                >
                  <span className={`text-sm ${isTodayDay ? 'text-white font-bold' : 'text-[#2C2C2C] font-normal'}`}>{format(day, 'eee', { locale: es })}</span>
                  <span className={`text-sm ${isTodayDay ? 'text-white font-bold' : 'text-[#2C2C2C] font-normal'}`}>{format(day, 'd')}</span>
                </div>
              );
            })}
          </div>
